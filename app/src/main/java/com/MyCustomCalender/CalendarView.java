...
import static com.MyCustomCalender.MonthIndex.FOCUSED_MONTH;
import static com.MyCustomCalender.MonthIndex.NEXT_MONTH;
import static com.MyCustomCalender.MonthIndex.PREVIOUS_MONTH;
...
public class CalendarView extends View {
    ...
    public Paint mSelectedDayCirclePaint;
    private Paint mCurrentDayCirclePaint;
    private float mCircleRadius;
    private Paint mEventCirclePaintGreen;
    private Paint mEventCirclePaintRed;
    ...
    public boolean isMonthScroll;
    private String defaultDate = "01";
    ...
    private void init() {
        ...
        mDetector = new GestureDetectorCompat(getContext(), new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onSingleTapUp(MotionEvent motionEvent) {
               /* if (!mScroller.isFinished()) {
                ...
                String date = year + "-" + monthString + "-" + selectedDate;
                //  lastSelectedDate = selectedDate;
                invalidate();
                dispatchOnDateSelected(date, calendarMonth.getEventOfDay(day));
                return true;
            }
        ...
        mSelectedDayCirclePaint.setTextSize(14 * getResources().getDisplayMetrics().density);
        ...
        mCurrentDayCirclePaint.setTextSize(14 * getResources().getDisplayMetrics().density);

        mEventCirclePaintRed = new Paint(Paint.ANTI_ALIAS_FLAG);
        mEventCirclePaintRed.setStyle(Paint.Style.FILL);
        mEventCirclePaintGreen = new Paint(Paint.ANTI_ALIAS_FLAG);
        mEventCirclePaintGreen.setStyle(Paint.Style.FILL);
    ...
    private void drawMonth(Canvas canvas, @MonthIndex int monthIndex) {
        ...
        if (monthIndex == FOCUSED_MONTH) {
            float[] crdCircle = calculateCrdForIndex(calendarMonth.getDayIndex(mMonthPager.getSelectedDay()),
                    null, monthIndex);
            canvas.drawCircle(crdCircle[0], crdCircle[1], 11 * getResources().getDisplayMetrics().density, mSelectedDayCirclePaint);
        }
        ...
        if (mMonthPager.isOnCurrentMonth(monthIndex)) {
            float[] crdCircle = calculateCrdForIndex(calendarMonth.getDayIndex(mMonthPager.getCurrentDay()),
                    null, monthIndex);
            canvas.drawCircle(crdCircle[0], crdCircle[1], 11 * getResources().getDisplayMetrics().density, mCurrentDayCirclePaint);
        }
    ...
    private void drawEventsOfDay(Canvas canvas, List<CalendarEvent> events, float[] crd, int day) {
        ...
        for (int i = 0; i < events.size(); i++) {
            mEventTextPaint.setColor(getResources().getColor(R.color.white));
            mEventTextPaint.setTextSize(10 * getResources().getDisplayMetrics().density);
            mEventCirclePaintRed.setColor(getResources().getColor(R.color.red));
            mEventCirclePaintGreen.setColor(getResources().getColor(R.color.bmh_green));
            canvas.drawCircle((float) (crd[0] - offsetForCenter + betweenPoints * (i + 0.3)),
                    (float) (crd[1] + mCircleRadius / 1.5), 9 * getResources().getDisplayMetrics().density, mEventCirclePaintRed);
            canvas.drawText(events.get(i).getText(), (float) (crd[0] - offsetForCenter + betweenPoints * (i + 0.05)),
                    crd[1] + mCircleRadius, mEventTextPaint);
            canvas.drawCircle((float) (crd[0] - offsetForCenter + betweenPoints * (i + 2)),
                    (float) (crd[1] + mCircleRadius / 1.5), 9 * getResources().getDisplayMetrics().density, mEventCirclePaintGreen);
            canvas.drawText(events.get(i).getTitle(), (float) (crd[0] - offsetForCenter + betweenPoints * (i + 1.7)),
                    crd[1] + mCircleRadius, mEventTextPaint);
        }
    }
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (mOffset == mScroller.getFinalX()) {
                ...
                CalendarMonth calendarMonth = mMonthPager.getCalendarMonth(FOCUSED_MONTH);
                dispatchOnDateSelected(String.valueOf(calendarMonth.getCalendar()),
    ...
    private void dispatchOnDateSelected(String calendar, List<CalendarEvent> eventsOfDay) {
    ...
    private void dispatchOnMonthChanged(Calendar calendar) {
        if (mOnMonthChangedListener != null) {
            int day = mMonthPager.getSelectedDay();
            //    CalendarMonth calendarMonth = mMonthPager.getCalendarMonth(FOCUSED_MONTH);
          ...
          /*
           ...
           /* String currDate;
            ...
            if (String.valueOf(day).length() == 1) {
                ...
                currDate = String.valueOf(day);
            }*/
            calendar.set(Integer.valueOf(year), monthInt, Integer.valueOf(stringDate));
            /*} else {
                calendar.set(Integer.valueOf(year), monthInt, Integer.valueOf(defaultDate));
            }*/
            setSelectedDayCircleColor(Color.TRANSPARENT);
