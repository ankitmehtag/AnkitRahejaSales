...
import com.adapters.BrokersListAdapter;
...
public class BrokersListActivity extends AppCompatActivity {
    public static String TAG = BrokersListActivity.class.getSimpleName();
    private Typeface font;
    private final int LOGIN_REQ_CODE_FOR_BROKER_LIST = 100;
    private ArrayList<BrokersListModel> mBrokersList = new ArrayList<>();
    private RecyclerView.LayoutManager layoutManager;
    private RecyclerView recyclerView;
    private BrokersListAdapter mAdapter = null;
    ...
    public void initViews() {
        ...
        toolbar.setTitle(R.string.txt_brokers);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        app = (BMHApplication) getApplication();
        gson = new Gson();
        listType = new TypeToken<ArrayList<BrokersListModel>>() {
        }.getType();
        recyclerView = findViewById(R.id.recyclerView_brokers);
    ...
    public boolean onCreateOptionsMenu(final Menu menu) {
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String s) {
                if (mBrokersList.size() > 0) {
                    final ArrayList<BrokersListModel> filteredList = filter(mBrokersList, s.toLowerCase());
        ...
        MenuItemCompat.setOnActionExpandListener(item,
                new MenuItemCompat.OnActionExpandListener() {
                    ...
                    public boolean onMenuItemActionCollapse(MenuItem item) {
                        if (mBrokersList.size() > 0)
                            mAdapter.setFilter(mBrokersList);
                        return true;
                    }
    ...
    private ArrayList<BrokersListModel> filter(List<BrokersListModel> models, String query) {
        query = query.toLowerCase();
        final ArrayList<BrokersListModel> filteredModelList = new ArrayList<>();
        for (BrokersListModel model : models) {
            final String brokerName = (model.getBroker_name()).toLowerCase();
            final String brokerType = (model.getBroker_type()).toLowerCase();
            final String brokerCode = (model.getBroker_code()).toLowerCase();

            if (brokerName.contains(query) || brokerCode.contains(query) || brokerType.contains(query)) {
    ...
    private void getBrokersData() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, WEBAPI.getWEBAPI(APIType.BROKERS_LIST),
                    ...
                    public void onResponse(String response) {
                        ...
                        try {
                            if (response != null) {
                                ...
                                if (mBrokersList.size() > 0) {
                                    mAdapter = new BrokersListAdapter(BrokersListActivity.this, mBrokersList);
                                    recyclerView.setAdapter(mAdapter);
                                    recyclerView.addItemDecoration(new MyDividerItemDecoration(BrokersListActivity.this, LinearLayoutManager.VERTICAL, 5));
                                    recyclerView.setItemAnimator(new DefaultItemAnimator());
                                    layoutManager = new LinearLayoutManager(BrokersListActivity.this);
                                    recyclerView.setLayoutManager(layoutManager);
                                } else {
                                    recyclerView.setBackgroundResource(R.drawable.no_broker_listed);
                                }
                            } else {
                                Toast.makeText(BrokersListActivity.this, R.string.txt_some_thing_went, Toast.LENGTH_SHORT).show();
                            }
    ...
    private void parseTransactionJson(JSONObject jsonObject) throws JSONException {
        ...
        mBrokersList = gson.fromJson(jsonArray.toString(), listType);
    }
