...
public class CitySearchAdapter extends BaseAdapter {
    ...
    private  final String TAG = CitySearchAdapter.class.getName();
    private Context mContext = null;
    private Typeface gothambook,icomoonfree = null;
    public ArrayList<CityDataModel> dataList;
    public ArrayList<CityDataModel> filterList = new ArrayList<>();

    public CitySearchAdapter(Context mContext, ArrayList<CityDataModel> dataList) {
    ...
    public void performFiltering(CharSequence constraint) {
        ...
        if(filterList != null){
            if (constraint.length() == 0) {
                ...
                for (final CityDataModel data : filterList) {
                    if (data.getCity_name() != null && data.getCity_name().toLowerCase().contains(filterPattern)) {
                        dataList.add(data);
                    }
                }
            }
   ...
   /* public class CustomFilter extends Filter {
       // private ProjectSearchAdapter mAdapter;
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            dataList = (ArrayList<CityDataModel>) results.values;
            notifyDataSetChanged();
        }
    }
*/
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(dataList != null && dataList.size() > position) {
            holder.tv_project.setText(dataList.get(position).getCity_name());
